...
export class ShortcutsService {
  ...
  private statePaintBucket: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  private stateMagnetism: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  ...
  private stateMouseScroll: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  currentStateMouseScroll: Observable<boolean> = this.stateMouseScroll.asObservable();

  onKeyPress(keyboard: KeyboardEvent): void {
    if (!this.turnOffShortcuts()) {
      switch (keyboard.code) {
        case Shortcuts.ONE_KEY:
          if (this.selectionService.controlsBoxCreated) {
        ...
        case Shortcuts.B_KEY:
          if (this.selectionService.controlsBoxCreated) {
            this.selectionService.removeControls();
          }
          this.iconService.updateSelectedIcon(TOPICONLIST[4]);
          this.setShortcutPaintBucket(true);
          keyboard.stopImmediatePropagation();
          break;

        case Shortcuts.M_KEY:
          this.iconService.updateSelectedIcon(TOPICONLIST[0]);
          this.setShortcutSelect(true);

          if (this.selectionService.isMagnetised) {
            this.selectionService.updateMagnetismState(false);
            this.setMagnetism(false);
          } else {
            this.selectionService.updateMagnetismState(true);
            this.setMagnetism(true);
          }
          keyboard.stopImmediatePropagation();
          break;
      }
  ...
  stateMouseScrollOn(mouseScrollOn: boolean): void {
    this.stateMouseScroll.next(mouseScrollOn);
  }
  ...
  setShortcutPaintBucket(statePaintBucket: boolean): void {
    this.statePaintBucket.next(statePaintBucket);
  }

  getShortcutPaintBucket(): Observable<boolean> {
    return this.statePaintBucket.asObservable();
  }
  ...
  setMagnetism(stateMagnetism: boolean): void {
    this.stateMagnetism.next(stateMagnetism);
  }

  getMagnetism(): Observable<boolean> {
    return this.stateMagnetism.asObservable();
  }
