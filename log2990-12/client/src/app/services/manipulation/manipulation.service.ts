...
export class ManipulationService {
  ...
  private altKeyResize(): void {
    if (this.keyAltOn) {
      let newNumber = 0;
      if (Math.sign(this.properties.amountTranslatedX) === -ONE && Math.sign(this.properties.amountTranslatedY) === -ONE) {
      ...
      } else if (Math.sign(this.properties.amountTranslatedX) === ONE && Math.sign(this.properties.amountTranslatedY) === ONE) {
        ...
        if (Math.abs(this.properties.amountTranslatedX) > Math.abs(this.properties.amountTranslatedY)) {
          if (Math.sign(this.properties.amountTranslatedY) === -ONE) {
            this.properties.amountTranslatedY = -(this.properties.amountTranslatedX);
          } else {
          this.properties.amountTranslatedY = this.properties.amountTranslatedX; }
        }
        if (Math.abs(this.properties.amountTranslatedX) < Math.abs(this.properties.amountTranslatedY)) {
          if (Math.sign(this.properties.amountTranslatedX) === -ONE) {
